...
#     return [{"file_name": rec[0], "resume_data": rec[1]} for rec in records]

# def clean_resume_data(resume):
#     """Sanitize resume data before indexing."""
#     for key, value in resume.items():
#         if isinstance(value, str) and value.strip().upper() == "N/A":
#             resume[key] = None
#     return resume

# def index_resumes_to_es():
#     resumes = fetch_resumes_from_db()
#     if resumes:
#         cleaned_resumes = [{"file_name": r["file_name"], "resume_data": clean_resume_data(r["resume_data"])} for r in resumes]
#         bulk_index_data(cleaned_resumes)
...
#     """
#     # For fields expected as arrays of strings.
#     list_fields = ["Skills", "Hobbies", "Languages", "Certifications", "Notable Companies"]
#     for field in list_fields:
#         if field in resume_data:
#             value = resume_data[field]
#             # if value is "N/A" (as string) or not a list, set to None
#             if isinstance(value, str):
#                 if value.strip().upper() == "N/A":
#                     resume_data[field] = None
#                 else:
#                     resume_data[field] = None  # or wrap in a list if needed: [value]
#             elif not isinstance(value, list):
#                 resume_data[field] = None

#     # For nested fields that should be lists of objects.
...
#                 # Ensure each element is a dict; if not, remove it.
#                 cleaned = []
#                 for item in value:
...
#     # Optionally, for any other field that is "N/A", set it to None.
